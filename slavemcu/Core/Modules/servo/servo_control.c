...
/**
 * @brief 初始化舵机控制（不校正，直接使用0-2526范围）
 */
void ServoControl_Init(ServoControl_t *ctrl, uint8_t id) {
    ...
    printf("[SERVO_CTRL] Working range: 0-2526 (0-222 degrees)\r\n");
}

/**
 * @brief 校正舵机位置（可选，设置当前位置为2048）
 * @note 对于222度应用，通常不需要校正
 */
void ServoControl_Calibrate(ServoControl_t *ctrl) {
    printf("[SERVO_CTRL] Calibrating servo ID=%d...\r\n", ctrl->id);
    printf("[SERVO_CTRL] WARNING: Calibration will limit range to ±180 degrees\r\n");
    
    // 使用扭矩寄存器的特殊功能校正位置为2048
    uint8_t torque_cal = 128;
    servo_write_reg(ctrl->id, SERVO_REG_TORQUE_ENABLE, &torque_cal, 1);
    HAL_Delay(100);
    ...
    // 验证校正
    uint16_t pos = servo_get_position(ctrl->id);
    printf("[SERVO_CTRL] Calibration complete. Position: %u\r\n", pos);
}
...
void ServoControl_SetTargetPosition(ServoControl_t *ctrl, int16_t position) {
    ...
    // 直接设置舵机位置
    servo_set_position(ctrl->id, position);
...
uint8_t ServoControl_IsAtTarget(ServoControl_t *ctrl) {
    return (abs(ctrl->position_error) < 20);  // 20步容差
}
...
void ServoControl_MoveToAngle(ServoControl_t *ctrl, float angle) {
    ...
    // 限制在0-2526范围内（0-222度）
    if (steps < 0) steps = 0;
    if (steps > 2526) steps = 2526;  // 222度上限
    
    printf("[SERVO_CTRL] Moving to angle %.1f° (steps: %d)\r\n", angle, steps);
...
void ServoControl_Home(ServoControl_t *ctrl) {
    printf("[SERVO_CTRL] Homing to 0 degrees (position 0)\r\n");
...
void ServoControl_PrintStatus(ServoControl_t *ctrl) {
    printf("[SERVO_STATUS] ID=%d, Pos=%d, Target=%d, Error=%d, Progress=%d%%\r\n",
           ctrl->id,
           ctrl->current_position,
