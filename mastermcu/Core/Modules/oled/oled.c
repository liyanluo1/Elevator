...
void Display_MotorStatus(Motor_t* motor) {
    ...
    u8g2_DrawStr(&u8g2, 0, 10, "Motor Status");

    // 当前位置
    u8g2_SetFont(&u8g2, u8g2_font_5x7_tr);
    sprintf(buffer, "Pos: %ld", Motor_GetPosition(motor));
    u8g2_DrawStr(&u8g2, 0, 22, buffer);

    // 目标位置
    sprintf(buffer, "Target: %ld", Motor_GetTargetPosition(motor));
    u8g2_DrawStr(&u8g2, 0, 32, buffer);

    // 速度
    sprintf(buffer, "Speed: %d", Motor_GetSpeed(motor));
    u8g2_DrawStr(&u8g2, 0, 42, buffer);

    // 运动状态
    const char* status_str;
    switch (Motor_GetStatus(motor)) {
        case MOTOR_STATUS_IDLE:
            status_str = Motor_IsMoving(motor) ? "Moving" : "Idle";
            break;
        case MOTOR_STATUS_RUNNING:
            status_str = "Running";
            break;
        case MOTOR_STATUS_FAULT:
            status_str = "Fault";
            break;
        case MOTOR_STATUS_TIMEOUT:
            status_str = "Timeout";
            break;
        default:
            status_str = "Unknown";
    ...
    sprintf(buffer, "Status: %s", status_str);
    u8g2_DrawStr(&u8g2, 0, 52, buffer);

    // CAN连接状态
    u8g2_DrawStr(&u8g2, 0, 62, Motor_IsConnected(motor) ? "CAN: OK" : "CAN: ERR");
...
void Display_Welcome(void) {
    ...
    u8g2_DrawStr(&u8g2, 15, 35, "CAN Motor");
...
void Display_SystemInfo(void) {
    ...
    u8g2_DrawStr(&u8g2, 0, 10, "System Info");

    sprintf(buffer, "Uptime: %lu s", HAL_GetTick() / 1000);
    u8g2_DrawStr(&u8g2, 0, 22, buffer);

    u8g2_DrawStr(&u8g2, 0, 34, "CAN: 500kbps");
    u8g2_DrawStr(&u8g2, 0, 46, "OLED: 128x64");
...
void Display_Error(const char* error_msg) {
    ...
    u8g2_SetFont(&u8g2, u8g2_font_ncenB08_tr);
    u8g2_DrawStr(&u8g2, 0, 15, "ERROR!");

    u8g2_SetFont(&u8g2, u8g2_font_6x10_tr);
    u8g2_DrawStr(&u8g2, 0, 30, error_msg);
